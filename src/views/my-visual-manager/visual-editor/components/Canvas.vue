<template>
  <div class="wrapper">
    <el-dialog title="发布" :visible.sync="$parent.publishPopVisible" width="50%">
      ...
      <el-input v-model="publicUrl" readonly /><span slot="footer">
        <el-button type="primary" @click="$parent.publishPopVisible = false">确 定</el-button></span>
    </el-dialog>
    <div
      class="edit-view"
      tabindex="0"
...
export default {
  ...
  computed: {
    publicUrl() {
      return `http://${window.location.host}${window.location.pathname}#/my-visual-manager/visual-view/${this.$route.params.id}`
    },
    chartData() {
