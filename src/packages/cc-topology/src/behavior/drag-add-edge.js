  ...
  options: {
    ...
    drawEdge: {
      ...
      start: function(event) {
        ...
        let sourceAnchor
        const sourceNodeModel = self.evtInfo.node.getModel()
        // 锚点数据
        const anchorPoints = self.evtInfo.node.getAnchorPoints()
        // 处理线条目标点
      ...
      stop(event) {
        ...
        if (self.drawEdge.isMoving) {
          if (self.drawEdge.currentLine === event.item) {
            ...
            // 锚点数据
            const anchorPoints = targetNode.getAnchorPoints()
            // 处理线条目标点
            if (anchorPoints && anchorPoints.length) {
              // 获取距离指定坐标最近的一个锚点
            ...
            if (this.historyData) {
              ...
              if (vm.undoCount > 0) {
                ...
                for (let i = 1; i <= vm.undoCount; i++) {
                  const key = `graph_history_${vm.historyIndex + i}`
                  vm.removeHistoryData(key)
                }
                vm.undoCount = 0
              } else {
                // 正常顺序执行的情况，记录拖拽前的数据状态
                const key = `graph_history_${vm.historyIndex}`
